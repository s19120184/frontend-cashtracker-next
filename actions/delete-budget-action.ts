...
export async  function deleteBudget(budgetId:Budget['id'],  preveData:ActionCreateBudget, formData: FormData){
    ...
    const checkPassworReq= await fetch(checkPasswordUrl, {
        method: 'POST',
        headers: { 
    ...
    const checkPasswordJson= await checkPassworReq.json()

    console.log(checkPasswordJson)

    if(!checkPassworReq.ok){
